...
## Advection Equation
The PDE we use is the advection equation:
```math
u_t(x,t) + c \cdot u_x(x,t)=0, \quad c\in\mathbb{R}.
```
The solution to the advection equation given the initial condition $u(x,0)=u_0(x)$ is given by:
```math
u(x,t) = u_0 (x-ct).
```
Our goal now is to reconstruct the velocity $c$ from given measurements, i.e., we "measure" $u$ at given grid points $x_i \in [a,b]$ and time steps $t_j \in [0,T]$. 

For the first experiment we choose $u_0(x)=\exp(-\frac{1}{2}x^2)$ and therefore
```math
u(x,t) = \exp(-\frac{1}{2}(x-ct)^2).
```
We choose to measure two time steps, i.e., $t_0=0$ and $t_1=1$ at ten equidistant grid points $x_i\in[-2,2]$ with $i\in[10]$.
"""
...
md"""
Plot our measurement for the start and end of the chosen time interval
"""
...
md"""
## Inverse Problem
To reconstruct the velocity $c$ from our measurments we use the forward problem.
```math
y=f(x).
```
In our case the forward problem can be formulated as
```math
y=u(x,t;c).
```

This is the simple case of a forward problem without noise. In general measurements are prone towards noise. One simple noise model is additive noise, where the noise $n$ is mutually independent of $x$. The forward problem then can be reformulated to
```math
y=f(x,n) \rightarrow y=u(x,t;c) + n, \quad \text{with } n\sim \mathcal{P}
```

Given our measurements $y_{i,j} = u(x_i,t_j;c)+n$, we can formulate an objective function we want to minimize with respect to our velocity $c$. We collect all measurements into a vector $y_{obs}=(y_{1,0},...,y_{10,0},y_{0,1},...,y_{10,1})$. The objective function we chose using our forward model $u(x,t)$ and our measurements $y_{obs}$ is
```math 
...
Assuming prior knwoledge on $C$ can be coded into probability density $\pi_{pr}(c)$ (**prior**). 

Assume we found the joint probability density of $Y,C$ after measurement, denoted by $\pi(c,y)$.
...
```math
\pi(c|y_{obs}) = \frac{\pi(c,y_{obs})}{\pi(y_{obs})}, \text{ if } \pi(y_{obs})=\int \pi(c,y_{obs})\mathrm{d}c \neq 0
```

Reformulated with Bayes' theorem of inverse problems:
```math
\pi_\mathrm{post}(c) = \pi(c|y_\mathrm{obs}) = \frac{\pi_\mathrm{pr}(c)\pi(y_\mathrm{obs}|c)}{\pi(y_\mathrm{obs})}
```

The goal now is to explore the posterior distribution to gain information about the unknown $c$.

### Explore the posterior
How do we get each part of the posterior?
#### Prior
Use prior knowledge we have on $C$. Somewhat complicated
### Likelihood
Straightforward. Likelihood function contains the forward model, information about the noise and other measurment and modelling uncertainties. 
We only consider additive independent noise first. Use distribution of noise $\pi_\mathrm{noise}(n)$
```math
Y = f(C;X,T) + N
```
Now given $C=c$ we can formulate $Y$ conditioned on $X=x$ as distributed like $N$
```math
\pi(y|c) = \pi_\mathrm{noise}(y-f(c;x,t))
```
Using $N\sim \mathcal{N}(0,\sigma)$, i.e., $\pi_\mathrm{noise}(n)=\frac{1}{\sqrt{2\pi\sigma^2}}\exp(-\frac{1}{2}\frac{n^2}{\sigma^2})$
```math
\pi(y|c) =  \frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{1}{2}\frac{(y-u(c;x,t))^2}{\sigma^2}\right)
```
"""
...
git-tree-sha1 = "d122dca3a00d40936bd3ee04c3c0c15ea9418274"
...
git-tree-sha1 = "c19523087706fe315f297252f54f3f6a2ae7ce85"
