...
fig = Figure()
ax1 = Axis(fig[1, 1], xlabel = "x [m]", ylabel = "z [m]", title = "Initial waterlevel")
lines!(ax1, data.x, data.H[1, :], color = :midnightblue, linewidth = 2, label = "h(x, 0)")
lines!(ax1, data.x, data.bathymetry, color = :brown, linewidth = 2, label = "b(x)")
scatter!(ax1, data.sensor_pos, data.H_sensor[1, :], color = :green, label = "sensor")
vlines!(ax1, data.sensor_pos, color = :black, linestyle = :dash, linewidth = 1)
