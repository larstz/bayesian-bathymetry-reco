...
σs = 0.5 .*deserialize(joinpath(lp_experiment, "log_posterior_grid_ss.jls"))
...
f = Figure()
ax = Axis(f[1, 1]; title="Log Posterior", xlabel="μ", ylabel="σ²", xticks=1.5:1.:15.0, yticks=0.0:0.05:1.0)
...
if length(ARGS) == 2
    ...
    save("log_posterior_with_chains.png", f)
    save("log_posterior_with_chains.pdf", f)
end
